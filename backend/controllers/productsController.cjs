  ...
  try {
    ...
    await pool.query(
      `INSERT INTO diamond.products 
        (id_product, name, current_price, description, type, stock, stock_min, stock_max, image)
  ...
  try {
    ...
    await pool.query(
      `UPDATE diamond.products SET
        ...
        stock=$5, stock_min=$6, stock_max=$7, image=$8
        WHERE id_product=$9`,
      [name, current_price, description, type, stock, stock_min, stock_max, image, id]
    );
